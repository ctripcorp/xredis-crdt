...
start_server {tags {"crdt-del"} overrides {crdt-gid 1} config {crdt.conf} module {crdt.so} } {
    ...
    test {"[crdt_del.tcl]concurrent crdt delete - 1"} {
        r set key-del-3 val
        after 1
        r CRDT.SET key-del-3 val2 3 [expr [clock milliseconds] - 10]  "1:10;2:99;3:100" 10000
    ...
    test {"[crdt_del.tcl]concurrent crdt delete - 2"} {
        ...
        after 1
        r CRDT.SET key-del-4 val2 3 [expr [clock milliseconds] - 10]  "1:10;2:99;3:100"  10000
        r get key-del-4
    } {}

    test {"[crdt_del.tcl]concurrent crdt delete - 3"} {
        r CRDT.DEL_REG key-del-5 2 [clock milliseconds] "1:30;2:100;3:100"
        r CRDT.SET key-del-5 val2 3 [expr [clock milliseconds] - 10]  "1:10;2:99;3:100"  10000
        r get key-del-5
    } {}
