...
start_server {tags {"repl"} config {crdt.conf} overrides {crdt-gid 1} module {crdt.so}} {
    ...
    start_server {config {crdt.conf} overrides {crdt-gid 2} module {crdt.so}} {
        ...
        start_server {config {crdt.conf} overrides {crdt-gid 3} module {crdt.so}} {
            ...
            #start writing
            test "psync data streaming ok" {
                write_batch_data [lindex $peer_hosts 0] [lindex $peer_ports 0]
                write_batch_data [lindex $peer_hosts 1] [lindex $peer_ports 1]
                write_batch_data [lindex $peer_hosts 2] [lindex $peer_ports 2]
