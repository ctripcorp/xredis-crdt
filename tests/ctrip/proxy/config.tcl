    ...
    start_server {tags {"crdt-set"} overrides {crdt-gid 2} config {crdt.conf} module {crdt.so} } {
        ...
        start_proxy {tags "xpipe-proxy"} {
            ...
            start_proxy {tags "xpipe-proxy2"} {
                ...
                test "master-peer2" {
                    $peer set test v
                    $master peerof $peer_gid $peer_host $peer_port proxy-type XPIPE-PROXY proxy-server [format "127.0.0.1:0,%s:%d" $proxy_host $proxy_port] proxy-params PROXYTLS://$proxy2_host:$proxy2_tls_port
    ...
    start_server {tags {"crdt-set"} overrides {crdt-gid 2} config {crdt.conf} module {crdt.so} } {
        ...
        start_proxy {tags "xpipe-proxy"} {
            ...
            start_proxy {tags "xpipe-proxy2"} {
                ...
                test "master-peer3" {
                    $peer set test v
                    $master peerof $peer_gid $peer_host $peer_port proxy-type XPIPE-PROXY proxy-server 127.0.0.1:0 proxy-params PROXYTLS://$proxy2_host:$proxy2_tls_port
                    $master config rewrite
    ...
    start_server {tags {"crdt-set"} overrides {crdt-gid 2} config {crdt.conf} module {crdt.so} } {
        ...
        start_proxy {tags "xpipe-proxy"} {
            ...
            start_proxy {tags "xpipe-proxy2"} {
                ...
                test "master-peer4" {
                    $peer set test v
                    $master peerof $peer_gid $peer_host $peer_port proxy-type XPIPE-PROXY proxy-server $proxy_host:$proxy_port proxy-params PROXYTLS://$proxy2_host:0
                    $master config rewrite
            ...
            start_proxy {tags "xpipe-proxy2"} {
                ...
                test "master-peer5" {
                    ...
                    start_proxy {tags "xpipe-proxy3"} {
                        ...
                        $master peerof $peer_gid $peer_host $peer_port proxy-type XPIPE-PROXY proxy-server 127.0.0.1:0,$proxy_host:$proxy_port proxy-params [format "PROXYTLS://%s:%d PROXYTLS://%s:%d" $proxy2_host $proxy2_tls_port $proxy3_host $proxy3_tls_port]
                        set sync_partial_ok_num [crdt_stats $peer sync_partial_ok]
                        wait_for_peer_sync $master
                    ...
                    $master peerof $peer_gid $peer_host $peer_port proxy-type XPIPE-PROXY proxy-server 127.0.0.1:0,$proxy_host:$proxy_port proxy-params [format "PROXYTLS://%s:%d;FORWARD_FOR 127.0.0.1:8888;" $proxy2_host $proxy2_tls_port ]
                    wait_for_peer_sync $master
