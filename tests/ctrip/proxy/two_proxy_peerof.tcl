    ...
    start_server {tags {"crdt-set"} overrides {crdt-gid 2} config {crdt.conf} module {crdt.so} } {
        ...
        start_proxy {tags "xpipe-proxy"} {
            ...
            start_proxy {tags "xpipe-proxy2"} {
                ...
                test "master-peer2" {
                    $master peerof $peer_gid $peer_host $peer_port proxy-type XPIPE-PROXY proxy-servers PROXYTCP://[format "%s:%d,%s:%d" {127.0.0.1} 0 $proxy_host $proxy_port] PROXYTLS://$proxy2_host:$proxy2_tls_port
                    wait_for_peer_sync $master
                    $peer set k v1 
                    after 1000
